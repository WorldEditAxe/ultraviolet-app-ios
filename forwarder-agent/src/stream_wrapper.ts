...
export class Wrapper extends EventEmitter {
    ...
    public async readPacket(channelId: number, packetId?: number): Promise<[number, Buffer]> {
        while (true) {
            const read = await this.readRaw(channelId),
                id = Protocol.readVarInt(read)
            if (id.value == packetId || packetId == null) {
                return [id.value, id.newBuffer]
            }
        }
    }
